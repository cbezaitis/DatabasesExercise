...
def traceUserInfuence(userId,depth):
    ...
    print("\n")
    lista=[]
    for i in range(len(friends_business)):
        kati=(1,friends_business[i][0],friends_business[i][1])
        lista.append(kati)
        print(kati)
    ...
    for vathos in range(2,depth+1):
        for i in range(len(lista)): 
            if lista[i][0]==(vathos-1): 
                pareSQL=sql_same_business(cur,lista[i][1],lista[i][2])
                for j in range(len(pareSQL)):
                    # na elegxo na min vazo ton idio tipo gia idia epixeirisi isos
                    # ean yparxei idi 
                    kati=(vathos,pareSQL[j][0],pareSQL[j][1])
                    lista.append(kati)

                

    print(lista)
    lista_me_onomata= list()
    for i in range(len(lista)):
        kati=(str(lista[i][1]),)
        lista_me_onomata.append(kati)
    print("\n to legnth tis listas")
    print(len(lista))
    lista_me_onomata=list(set(lista_me_onomata))
    lista_me_onomata.sort()
    lista_me_onomata.insert(0,(("influenced users"),))
    lista_me_onomata=tuple(lista_me_onomata)
    return lista_me_onomata
